...
def top_artists():
    ...
    headers = {
        "x-rapidapi-key": "c40c257b90msh431a7e8dacdd216p122e38jsn54623865cda6",
    ...
    if 'artists' in response_data:
        for artist in response_data['artists']:
            ...
            avatar_url = artist.get('visulas', {}).get('avatar', [{}])[0].get('url', 'No URL')
...
def top_tracks():
    ...
    if 'tracks' in data:
        ...
        for track in shortened_data:
            ...
            artist_name = track['artist'][0]['name'] if track['artists'] else None
...
def get_track_image(track_id, track_name):
    ...
    r = requests.get(url)
    soup = bs(r.content)
    image_link_html = soup.find('img', {'alt': track_name})
    if image_link_html:
        image_links = image_link_html['srcset']
    else:
        image_links = ''
